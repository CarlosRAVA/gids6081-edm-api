...
export class UserController {
    ...
    @Get()
    async getAllUser(): Promise<User[]> { //es importante especificar que tipo de dato se esta retornando
    ...
    public async listUserById(@Param("id", ParseIntPipe) id: number): Promise<User> {
        ...
        if (result == undefined)
            throw new HttpException(`Usuario con ID no encontrado`, HttpStatus.NOT_FOUND); //mensaje de error por HTTP, con mensaje personalizado
